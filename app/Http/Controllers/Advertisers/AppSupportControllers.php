...
class AppSupportControllers extends Controller
{
    ...
    public function infoTest(Request $request)
    {
        ...
        if ($support) {
            $supportLog =  DB::table('support_logs')
                    ->join('users', 'support_logs.user_id', '=', 'users.uid')
                    ->select(DB::raw("CONCAT(ss_users.first_name, ' ', ss_users.last_name) AS user_name"), 'support_logs.created_by', 'support_logs.ticket_no', 'support_logs.user_id', 'support_logs.message', 'support_logs.file', 'support_logs.created_at', 'support_logs.status')
                    ->where('support_logs.user_id', $uid)
                    ->where('support_logs.ticket_no', $ticketno)
                    ->orderBy('support_logs.id', 'DESC')
                    ->get();
    ...
    public function info(Request $request)
    {
        ...
        $support = Supports::select('category', 'sub_category', 'support_type', 'subject', 'message', 'file', 'status')
            ->where('uid', $uid)->where('ticket_no', $ticketno)->orderBy('id', 'DESC')->first();
        if ($support) {
            $supportLog = SupportLog::select(DB::raw("CONCAT(ss_users.first_name,' ',ss_users.last_name) as user_name"), 'support_logs.created_by','support_logs.ticket_no','support_logs.user_id','support_logs.message','support_logs.file','support_logs.created_at','support_logs.status','support_logs.user_name as created_by')
