...
class AppAuthControllers extends Controller
{
    ...
    {
        $user = User::select('first_name', 'last_name', 'email', 'phone', 'address_line1', 'address_line2', 'city', 'state', 'country', 'pub_wallet', 'profile_lock','phonecode','messenger_type','messenger_name','user_type')->where('uid', $uid)->first();
        if ($user) {
            $return['code']    = 200;
            $return['data']    = $user;
            $wltAmt = getWalletAmount();
