...

{
    ...
    public function transactionsReport(Request $request)
    {
        ...
        if ($data) {
            ...
            $return['data']                = $data;
            $return['row']                 = $row;
            $return['totalSuccessAmoutn']  = $totalsuccamt;
...

    {
        ...
        $getUser = DB::table('users')->select('account_type')->where('uid',$uid)->where('status', 0)->where('trash', 0)->first();
...

        {
            ...
            $paybleamt     = $txnupdate->amount + $txnupdate->fee + $txnupdate->gst;
            ...
            $noti_title = 'Payment Successful ';
            $noti_desc = '#'.$transactionid. '  '. 'Your payment has been successfully completed. The amount is added to your wallet.';
