...
class ReportAdminController extends Controller
{
    ...
    public function cmpreport(Request $request)
    {
        ...
        if ($startDate && $endDate) {
            ...
            // $campaign->whereDate('ad_impressions.created_at', '>=', $nfromdate)
            // ->whereDate('ad_impressions.created_at', '<=', $endDate);
        }
        if ($src) {
        ...
        //     $campaign->whereDate('campaigns.created_at', '>=', $nfromdate)
        //     ->whereDate('campaigns.created_at', '<=', $endDate)->whereRaw('concat(ss_users.uid,ss_campaigns.campaign_id,ss_campaigns.campaign_name) like ?', "%{$src}%");
        // }
  	...
  	public function cmpReportExportDateWise(Request $request)
    {
        ...
        $campaign = DB::table('campaigns')
            ...
            // $campaign = DB::table('campaigns')
            // ->select(DB::raw("CONCAT(ss_users.first_name, ' ', ss_users.last_name) as name,  
            // (select count(id) from ss_user_camp_click_logs camp_ck where camp_ck.campaign_id = ss_campaigns.campaign_id) as click, 
            // (select sum(amount) from ss_user_camp_click_logs camp_ck where camp_ck.campaign_id = ss_campaigns.campaign_id) as clickamt, 
            // (select count(id) from ss_ad_impressions ad_imp where ad_imp.campaign_id = ss_campaigns.campaign_id) as imprs, 
            // (select sum(amount) from ss_ad_impressions ad_imp where ad_imp.campaign_id = ss_campaigns.campaign_id) as imprsamt"), 
            // 'campaigns.id', 'campaigns.campaign_name', 'campaigns.campaign_id', 'campaigns.advertiser_code','campaigns.status', 
            // 'campaigns.trash', 'campaigns.ad_type', 'campaigns.created_at', 'users.account_type', 'categories.cat_name')
            // ->join('users', 'campaigns.advertiser_code', '=', 'users.uid')
            // ->join('categories', 'campaigns.website_category', '=', 'categories.id')
            // ->whereDate('campaigns.created_at', '>=', $nfromdate)
