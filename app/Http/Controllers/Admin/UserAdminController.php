...
{
    public function usersListOld(Request $request)
    {
        ...
        $userlist = DB::table('users')
            ...
            (select count(id) from ss_campaigns camp_ck where camp_ck.advertiser_code = ss_users.uid AND trash=0 ) as cmpcount"), 
            'users.email', 'users.user_type', 'users.website_category', 'users.status', 'users.ac_verified', 'users.country', 'users.created_at', 'users.uid', 'categories.cat_name')
            ->where('users.trash', 0)
        ...
        $data = $userlist->orderBy('users.id', 'desc')->get();
    ...
    public function addnewusers(Request $request)
    {
        $validator = Validator::make(
            ...
            [
                ...
                'phone'  => 'required|numeric|unique:users,phone,max:15',
                'user_type'  => 'required',
