...
class AppPubReportUserController extends Controller
{
        public function ad_report(Request $request)
    {
      ...
      //$row   = $sql->count();
      $datas = $sql->orderBy('pub_stats.udate', 'DESC')->get();
      $row   = count($datas);
  	...
  	public function payoutMethodListTest(Request $request)
    {
      $validator = Validator::make($request->all(), [
        'uid'       => 'required',
      ]);
      ...
      { 
        if(!empty($listmode) && trim($listmode->payout_name) === trim($list->method_name)){
          $minAmt = $list->min_withdrawl;
        }
